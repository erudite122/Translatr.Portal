...
using Translatr.Portal.Migrations.SeedData;
...
namespace Translatr.Portal.Tests
{
    public abstract class PortalTestBase : AbpIntegratedTestBase
    {
        ...
        protected Task UsingDbContextAsync(Action<PortalDbContext> action)
        {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<PortalDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PortalDbContext>())
